...
def grade(action: Action, ground_truth: Dict[str, Any]) -> float:
    ...
    for e in entries:
        ...
        if e.status == expected:
            ...
            if expected == "MISMATCH" and e.mismatch_fields:
                valid_reported = [
                    f for f in e.mismatch_fields if f in _VALID_MISMATCH_FIELDS
                ]
                if e.mismatch_fields:
                    field_score_total += min(
                        1.0, len(valid_reported) / len(e.mismatch_fields)
                    )
                    field_entry_count += 1
    ...
    field_bonus = (
        field_score_total / field_entry_count if field_entry_count > 0 else 0.0
    )
