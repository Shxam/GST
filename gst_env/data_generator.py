...
def generate_gstin(
    ...
    state_prefix = "{:02d}".format(state_code)
    ...
    assert len(gstin) == 15, "Generated GSTIN length {}: {}".format(len(gstin), gstin)
    return gstin
...
def generate_invoice(invoice_id: str, seed: int) -> Dict[str, Any]:
    ...
    invoice = Invoice(
        ...
        vendor_gstin=gstin,
        invoice_number="INV-{}".format(invoice_id),
...
def generate_task1_data(seed: int = 42) -> Dict[str, Any]:
    ...
    for i in range(10):
        inv = generate_invoice("T1-{:04d}".format(i), seed + i)
        invoices.append(inv)
...
def generate_task2_data(seed: int = 42) -> Dict[str, Any]:
    ...
    for i in range(50):
        inv = generate_invoice("T2-{:04d}".format(i), seed + i)
        invoices.append(inv)
   ...
   #
   # max_itc = sum(_calc_itc(inv) for inv in invoices)
...
def generate_task3_data(seed: int = 42) -> Dict[str, Any]:
    ...
    for i in range(200):
        inv = generate_invoice("T3-{:04d}".format(i), seed + i)
        invoices.append(inv)
