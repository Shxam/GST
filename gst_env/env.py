...
class GSTReconciliationEnv:
    ...
    def reset(self, task_id: str) -> Observation:
        if task_id not in TASK_GENERATORS:
            raise ValueError(
                "Unknown task_id {!r}. Valid: {}".format(
                    task_id, list(TASK_GENERATORS.keys())
                )
            )
        ...
        # Store extra fields graders need under consistent keys
    ...
    def step(
        ...
        self._step_number += 1
        self._done = True  # single-step environment
        ...
        # Return same obs (environment is single-step)
